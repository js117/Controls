...
V = 100; % in practice we don't know gsigma exactly, so let's say we underestimate it
vbar = gbar;
W = 1*[1,0;0,1]; %"process noise"
wbar = 0;
y_data = zeros(p,numTimeSteps);
y_data(1,:) = g;

% We can use this one line to call the Kalman Filter routine, or the
% commented-out below version which is pretty much a verbatim copy of that
% code. 
